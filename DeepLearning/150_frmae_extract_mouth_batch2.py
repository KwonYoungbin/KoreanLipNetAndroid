...
for filepath in find_files(SOURCE_PATH, SOURCE_EXTS):
    ...
    for frame in video.mouth-1:
    	io.imsave(os.path.join(target_dir, "mouth_{0:03d}.png".format(i+75)), frame)
    	i += 1


    copy_img = target_dir + '/mouth_075.png'
    dest_dir = target_dir

    j = 1

    for j in range(74):
	shutil.copy2(copy_img, dest_dir + '/mouth_{0:03d}.png'.format(j+1))
